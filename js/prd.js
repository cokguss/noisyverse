(() => {
  ...
  function buildAiPrompt() {
    ...
    parts.push("");
    // ATURAN KELENGKAPAN: model punya batas token output, jadi kalau ia menulis
    // satu bagian terlalu panjang, dokumen terpotong di tengah dan bagian akhir
    // (skema DB, endpoint, KPI, risiko, milestone, pertanyaan terbuka) hilang —
    // itulah keluhan "PRD belum lengkap". Karena itu instruksinya menegaskan
    // KELENGKAPAN semua bagian di atas kedalaman satu bagian, plus target panjang
    // per bagian supaya seluruh dokumen pasti selesai.
    parts.push(lang === "en"
      ? "Output rules: Write the ENTIRE document in English. Output raw Markdown ONLY (no explanations before/after). Must contain these sections IN ORDER: # title, metadata blockquote, product overview, background & problem, goals & non-goals, target users table, user stories list, features with MoSCoW priorities (Must/Should/Could), non-functional requirements, UI/UX guidelines, technology & stack with reasoning, ARCHITECTURE & PROJECT FOLDER STRUCTURE (folder tree in a code block with a short note per folder), DATABASE SCHEMA (main tables with columns & relations in markdown tables), API ENDPOINTS LIST (method, path, description in a table), success metrics KPIs, risks & mitigation table, release milestones, open questions. Be specific and concrete — no placeholders, no lorem ipsum. HARD LENGTH LIMITS (the model has a token cap, so a too-long doc gets cut off before the final sections — that is the top failure to avoid): folder tree MAX 12 lines, every table MAX 6 rows, every prose section MAX 4 lines, whole document about 1200-1600 words. CRITICAL: it is far better to have ALL sections present but very brief than to have some sections missing. If space runs low, keep every remaining section (KPIs, risks, milestones, open questions) even if only one line each, and MAKE SURE the document finishes all the way through 'open questions' without being cut off."
      : "Aturan output: Tulis SELURUH dokumen dalam Bahasa Indonesia. Keluarkan raw Markdown SAJA (tanpa penjelasan sebelum/sesudah). Wajib memuat bagian berikut BERURUTAN: # judul, blockquote metadata, ringkasan produk, latar belakang & masalah, tujuan & non-tujuan, tabel target pengguna, daftar user stories, fitur dengan prioritas MoSCoW (Must/Should/Could), persyaratan non-fungsional, panduan UI/UX, teknologi & stack beserta alasannya, ARSITEKTUR & STRUKTUR FOLDER PROYEK (pohon folder dalam code block beserta catatan singkat per folder), SKEMA DATABASE (tabel utama beserta kolom & relasinya dalam markdown table), DAFTAR API ENDPOINTS (method, path, deskripsi dalam tabel), metrik sukses KPI, tabel risiko & mitigasi, milestone rilis, pertanyaan terbuka. Isi spesifik dan konkret — tanpa placeholder, tanpa lorem ipsum. BATAS PANJANG KETAT (model punya batas token, jadi dokumen yang kepanjangan terpotong sebelum bagian akhir — inilah kegagalan utama yang harus dihindari): pohon folder MAKSIMAL 12 baris, setiap tabel MAKSIMAL 6 baris, setiap bagian narasi MAKSIMAL 4 baris, total dokumen sekitar 1200-1600 kata. SANGAT PENTING: jauh lebih baik SEMUA bagian ada tapi sangat ringkas daripada ada bagian yang hilang. Kalau ruang menipis, tetap tulis semua bagian sisa (KPI, risiko, milestone, pertanyaan terbuka) walau satu baris saja, dan PASTIKAN dokumen selesai sampai bagian \"pertanyaan terbuka\" tanpa terpotong.");
    return parts.join("\n");
  }
